...
def _put_trans_data(transCmd, parameters, data, noPad=False):
    ...
    if len(parameters):
        ...
        transData = (b'\x00' * padLen) + (parameters if isinstance(parameters, bytes) else parameters.encode())
        offset += padLen + len(parameters)

    if len(data):
        padLen = 0 if noPad else (4 - offset % 4 ) % 4
        transCmd['Parameters']['DataOffset'] = offset + padLen
        transData += (b'\x00' * padLen) + (data if isinstance(data, bytes) else data.encode())
